...
def custom_padding(input_tensor):
    ...
    height_shift = np.random.choice((0,1))
    width_shift = np.random.choice((0,1))
...
class Model(nn.Module):
    ...
    def forward(self, galaxy_images_output, ebv_output):
        ...
        conv2d = nn.Conv2d(in_channels=5, out_channels=64, kernel_size=(5,5), padding=2, bias=True)
        ...
        conv2d2 = nn.Conv2d(in_channels=64, out_channels=48, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output2 = conv2d2(averagepooling2d_output)

        prelu2 = nn.PReLU(init=0.0)
        prelu_output2 = prelu2(conv2d_output2)

        conv2d3 = nn.Conv2d(in_channels=48, out_channels=64, kernel_size=(3, 3), padding=1, bias=True)
        conv2d_output3 = conv2d3(prelu_output2)

        prelu3 = nn.PReLU(init=0.0)
        prelu_output3 = prelu3(conv2d_output3)

        conv2d4 = nn.Conv2d(in_channels=64, out_channels=48, kernel_size=(1, 1), padding=0, bias=True)
        ...
        conv2d5 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output5 = conv2d5(averagepooling2d_output)

        prelu5 = nn.PReLU(init=0.0)
        prelu_output5 = prelu5(conv2d_output5)

        conv2d6 = nn.Conv2d(in_channels=64, out_channels=48, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output6 = conv2d6(averagepooling2d_output)

        prelu6 = nn.PReLU(init=0.0)
        prelu_output6 = prelu6(conv2d_output6)

        conv2d7 = nn.Conv2d(in_channels=48, out_channels=64, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output7 = conv2d7(prelu_output6)

        prelu7 = nn.PReLU(init=0.0)
        prelu_output7 = prelu7(conv2d_output7)

        concatenate_output = torch.cat((prelu_output5, prelu_output3, prelu_output7, averagepooling2d_output2), dim=1)

        conv2d8 = nn.Conv2d(in_channels=240, out_channels=64, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output8 = conv2d8(concatenate_output)

        conv2d9 = nn.Conv2d(in_channels=240, out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output9 = conv2d9(concatenate_output)

        conv2d10 = nn.Conv2d(in_channels=240, out_channels=64, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output10 = conv2d10(concatenate_output)

        prelu8 = nn.PReLU(init=0.0)
        prelu_output8 = prelu8(conv2d_output10)

        conv2d11 = nn.Conv2d(in_channels=64, out_channels=92, kernel_size=(5, 5), padding=2, bias=True)
        ...
        conv2d12 = nn.Conv2d(in_channels=240, out_channels=64, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output12 = conv2d12(concatenate_output)

        prelu11 = nn.PReLU(init=0.0)
        prelu_output11 = prelu11(conv2d_output12)

        conv2d13 = nn.Conv2d(in_channels=64, out_channels=92, kernel_size=(3, 3), padding=1, bias=True)
        ...
        concatenate_output2 = torch.cat((prelu_output13, prelu_output12, prelu_output9, averagepooling2d_output3), dim=1)

        averagepooling2d4 = nn.AvgPool2d((2, 2), stride=2, padding=0)
        averagepooling2d_output4 = averagepooling2d4(concatenate_output2)

        conv2d14 = nn.Conv2d(in_channels=340, out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output14 = conv2d14(averagepooling2d_output4)

        prelu14 = nn.PReLU(init=0.0)
        prelu_output14 = prelu14(conv2d_output14)

        conv2d15 = nn.Conv2d(in_channels=92, out_channels=128, kernel_size=(5, 5), padding=2, bias=True)
        conv2d_output15 = conv2d15(prelu_output14)

        prelu15 = nn.PReLU(init=0.0)
        prelu_output15 = prelu15(conv2d_output15)

        conv2d16 = nn.Conv2d(in_channels=340, out_channels=128, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output16 = conv2d16(averagepooling2d_output4)

        prelu16 = nn.PReLU(init=0.0)
        prelu_output16 = prelu16(conv2d_output16)

        conv2d17 = nn.Conv2d(in_channels=340, out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        ...
        conv2d18 = nn.Conv2d(in_channels=340, out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output18 = conv2d18(averagepooling2d_output4)

        prelu18 = nn.PReLU(init=0.0)
        prelu_output18 = prelu18(conv2d_output18)

        conv2d19 = nn.Conv2d(in_channels=92, out_channels=128, kernel_size=(3, 3), padding=1, bias=True)
        conv2d_output19 = conv2d19(prelu_output18)

        prelu19 = nn.PReLU(init=0.0)
        prelu_output19 = prelu19(conv2d_output19)

        concatenate_output3 = torch.cat((prelu_output16, prelu_output19, prelu_output15, averagepooling2d_output5), dim=1)

        conv2d20 = nn.Conv2d(in_channels=476,out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output20 = conv2d20(concatenate_output3)

        prelu20 = nn.PReLU(init=0.0)
        prelu_output20 = prelu20(conv2d_output20)

        conv2d21 = nn.Conv2d(in_channels=92,out_channels=128, kernel_size=(3, 3), padding=1, bias=True)
        conv2d_output21 = conv2d21(prelu_output20)

        prelu21 = nn.PReLU(init=0.0)
        prelu_output21 = prelu21(conv2d_output21)

        conv2d22 = nn.Conv2d(in_channels=476,out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        ...
        conv2d23 = nn.Conv2d(in_channels=476,out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output23 = conv2d23(concatenate_output3)

        prelu23 = nn.PReLU(init=0.0)
        prelu_output23 = prelu23(conv2d_output23)

        conv2d24 = nn.Conv2d(in_channels=92,out_channels=128, kernel_size=(5, 5), padding=2, bias=True)
        conv2d_output24 = conv2d24(prelu_output23)

        prelu24 = nn.PReLU(init=0.0)
        prelu_output24 = prelu24(conv2d_output24)

        conv2d25 = nn.Conv2d(in_channels=476,out_channels=128, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output25 = conv2d25(concatenate_output3)

        prelu25 = nn.PReLU(init=0.0)
        prelu_output25 = prelu25(conv2d_output25)

        concatenate_output4 = torch.cat((prelu_output25, prelu_output21, prelu_output24, averagepooling2d_output6), dim=1)

        averagepooling2d7 = nn.AvgPool2d((2, 2), stride=2, padding=0)
        averagepooling2d_output7 = averagepooling2d7(concatenate_output4)

        conv2d26 = nn.Conv2d(in_channels=476,out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        ...
        conv2d27 = nn.Conv2d(in_channels=476,out_channels=92, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output27 = conv2d27(averagepooling2d_output7)

        prelu27 = nn.PReLU(init=0.0)
        prelu_output27 = prelu27(conv2d_output27)

        conv2d28 = nn.Conv2d(in_channels=92,out_channels=128, kernel_size=(3, 3), padding=1, bias=True)
        conv2d_output28 = conv2d28(prelu_output27)

        prelu28 = nn.PReLU(init=0.0)
        prelu_output28 = prelu28(conv2d_output28)

        conv2d29 = nn.Conv2d(in_channels=476,out_channels=128, kernel_size=(1, 1), padding=0, bias=True)
        conv2d_output29 = conv2d29(averagepooling2d_output7)

        prelu29 = nn.PReLU(init=0.0)
        prelu_output29 = prelu29(conv2d_output29)

        concatenate_output5 = torch.cat((prelu_output29, prelu_output28, averagepooling2d_output8), dim=1)
...
model = Model()
model(galaxy_images_output, ebv_output)
